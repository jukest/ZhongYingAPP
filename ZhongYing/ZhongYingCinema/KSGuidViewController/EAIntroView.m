...
- (void)buildScrollViewWithFrame:(CGRect)frame {
    ...
    for (int idx = 0; idx < _pages.count; idx++) {
        ...
        if (idx == _pages.count -1) {
            UIView *view = pageViews[idx];
            UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hideWithFadeOutDuration:)];
...
- (void)buildFooterView {
    ...
    self.pageControl.pageIndicatorTintColor = COLOR_NAVAAR;
    self.pageControl.currentPageIndicatorTintColor = [UIColor grayColor];
    [self addSubview:self.pageControl];
    
    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(self.scrollView.frame.size.width/2-250/2, self.frame.size.height/3*2, 250, 150)];
...
- (void)skipIntroduction {
    if (self.scrollView.contentOffset.x/self.scrollView.frame.size.width == [_pages count]-1) {
        if ([(id)self.delegate respondsToSelector:@selector(introDidFinish)]) {
            [self.delegate introDidFinish];
        }
