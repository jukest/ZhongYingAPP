...
- (void)initialize{
    ...
    self.imageView = [FanShuToolClass createImageViewWithFrame:self.bounds color:[UIColor grayColor]];
    [superViewOfScrollView addSubview:self.imageView];
    ...
    self.view = [FanShuToolClass createViewWithFrame:CGRectMake(0, 0, 97, 132) backgroundColor:[UIColor whiteColor]];
    self.view.center = CGPointMake(superViewOfScrollView.frame.size.width / 2, superViewOfScrollView.frame.size.height / 2);
    [superViewOfScrollView addSubview:self.view];
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        switch (self.orientation) {
            case NewPagedFlowViewOrientationHorizontal:
                cell.frame = CGRectMake((_pageSize.width +100) * pageIndex, 0, _pageSize.width, _pageSize.height);
                break;
            case NewPagedFlowViewOrientationVertical:
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if ((int)scrollView.contentOffset.x % (int)_pageSize.width == 0 && !self.Dragging) {
        self.view.hidden = NO;
    }
    ...
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:
            pageIndex = (int)roundf(_scrollView.contentOffset.x / _pageSize.width) % self.orginPageCount;
            break;
        case NewPagedFlowViewOrientationVertical:
            pageIndex = (int)roundf(_scrollView.contentOffset.y / _pageSize.height) % self.orginPageCount;
            break;
        default:
            break;
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if ([_delegate respondsToSelector:@selector(didScrollToPage:inFlowView:)] && _currentPageIndex != _index) {
        [_delegate didScrollToPage:_index inFlowView:self];
    }
