...
#define kYSLScrollMenuViewWidth ScreenWidth / self.itemTitleArray.count
...
- (void)setItemTitleArray:(NSArray *)itemTitleArray{
    if (_itemTitleArray != itemTitleArray) {
        ...
        for (NSInteger i = 0; i < itemTitleArray.count -1; i ++) {
            UIView *line = [FanShuToolClass createViewWithFrame:CGRectMake(ScreenWidth / itemTitleArray.count * (i +1), 10, 1, CGRectGetHeight(self.frame)-20) backgroundColor:Color(244, 244, 244, 1.0)];
...
- (void)setItemTextColor:(UIColor *)itemTextColor seletedItemTextColor:(UIColor *)selectedItemTextColor currentIndex:(NSInteger)currentIndex{
    ...
    for (int i = 0; i < self.itemViewArray.count; i++) {
        ...
        if (i == currentIndex) {
            label.alpha = 0.0;
            [UIView animateWithDuration:0.75 delay:0.0 options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionAllowUserInteraction animations:^{
                label.alpha = 1.0;
